...
{
  config = lib.mkMerge [
    (lib.mkIf (cfg.enable && cfg.hypridle.enable) {
      assertions = [
        {
          assertion = hyprlandEnabled;
          message = "homeconfig.gui.hypridle.enable requires Hyprland to be enabled.";
        }
        {
          assertion = noctaliaEnabled;
          message = "homeconfig.gui.hypridle.enable currently uses Noctalia as its locker.";
        }
      ];
    })

    (lib.mkIf (cfg.enable && cfg.hypridle.enable && hyprlandEnabled && noctaliaEnabled) {
      services.hypridle = {
        enable = true;
