...
After installing NixOS (step 5), extract the host's age public key from its
...
Shared workstation hosts import the impermanence module, but the root reset is
disabled by default. Enable `nixconfig.storage.impermanence.enable` only in the
specific host after its persistent backing state has been staged.

Before switching an existing host with impermanence enabled, verify that the
storage layout matches the shared disko layout:
...
Every command must succeed. `root@blank` is supposed to be a blank root dataset
snapshot, created before the root dataset is populated. Do not create it from a
currently running, populated root filesystem and call that blank. If the root
dataset, blank snapshot, or persistent datasets are missing, do not switch the
host with impermanence enabled yet. Treat the host as `migrationRequired` only
if you have an explicit in-place ZFS migration plan; otherwise use the reinstall
path so disko creates the expected layout and blank snapshot.
...
The inventory must include `/etc/machine-id`, a stable `networking.hostId`, and
any enabled service state that cannot be safely regenerated. Runtime
`/run/secrets` files are regenerated by sops-nix and are not persistent source
data.

SSH host identity is not managed through impermanence. It lives directly under
`/persist/etc/ssh`; both sops-nix and OpenSSH read that location so the machine
has one canonical host identity with or without impermanence enabled.

Copy every listed file and directory into the matching backing path under
`/persist` before enabling impermanence. At minimum:
...
snapshot, persistent backing state, or reviewed system-state reasons are
missing. Fix the readiness issue before deploying; do not bypass the checks by
disabling individual assertions.
...
The installer will:
- Decrypt SSH host keys from `secrets/hosts/newhostname/ssh/` to
  `extra-files/newhostname/persist/etc/ssh/`
- Partition and format the disk (via disko)
- Generate `facter.json` hardware config
- Copy the SSH host keys into persistent storage
